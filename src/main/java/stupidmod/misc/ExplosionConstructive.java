...
public class ExplosionConstructive {
    
    public Explosion doppelganger;
    ...
    @OnlyIn(Dist.CLIENT)
    public ExplosionConstructive(World worldIn, @Nullable Entity entityIn, double x, double y, double z, float size, List<BlockPos> affectedPositions) {
        this(worldIn, entityIn, x, y, z, size, false, true, affectedPositions);
    }
    
    @OnlyIn(Dist.CLIENT)
    public ExplosionConstructive(World worldIn, @Nullable Entity entityIn, double x, double y, double z, float size, boolean causesFire, boolean damagesTerrain, List<BlockPos> affectedPositions) {
        this(worldIn, entityIn, x, y, z, size, causesFire, damagesTerrain, Blocks.AIR.getDefaultState());
        this.affectedBlockPositions.addAll(affectedPositions);
    }
    
    public ExplosionConstructive(World worldIn, @Nullable Entity entityIn, double x, double y, double z, float size, boolean causesFire, boolean damagesTerrain, IBlockState blockState) {
        doppelganger = new Explosion(worldIn, entityIn, x, y, z, size, causesFire, damagesTerrain);
        ...
        this.damageSource = DamageSource.causeExplosionDamage(this.doppelganger);
    ...
    public void doExplosionA() {
                    ...
                    if (j == 0 || j == 15 || k == 0 || k == 15 || l == 0 || l == 15) {
                        ...
                        for(float f1 = 0.3F; f > 0.0F; f -= 0.22500001F) {
                            ...
                            if (!iblockstate.isAir(world, blockpos) || !ifluidstate.isEmpty()) {
                                float f2 = Math.max(iblockstate.getExplosionResistance(world, blockpos, exploder, this.doppelganger), ifluidstate.getExplosionResistance(world, blockpos, exploder, this.doppelganger));
                                if (this.exploder != null) {
                                    f2 = this.exploder.getExplosionResistance(this.doppelganger, this.world, blockpos, iblockstate, ifluidstate, f2);
                                }
                                
                                f -= (f2 + 0.3F) * 0.3F;
                            }
                            
                            if (f > 0.0F && (this.exploder == null || this.exploder.canExplosionDestroyBlock(this.doppelganger, this.world, blockpos, iblockstate, f))) {
                                set.add(blockpos);
                            }
        ...
        Class expClass = this.doppelganger.getClass();

        try {
            Field f = expClass.getDeclaredField("affectedBlockPositions");
            f.setAccessible(true);
            f.set(this.doppelganger, this.affectedBlockPositions);
        }
        catch (NoSuchFieldException | IllegalAccessException notGonnaHappen) { System.out.println("REFLECTION ERROR!!!!!!!!!!!!!!!!!!!!!!!!!!!"); }

        net.minecraftforge.event.ForgeEventFactory.onExplosionDetonate(this.world, this.doppelganger, list, f3);
    ...
    public void doExplosionB(boolean spawnParticles) {
        ...
        if (this.damagesTerrain) {
            for(BlockPos blockpos : this.affectedBlockPositions) {
                ...
                if (iblockstate.getBlock() != blockState.getBlock()) {
                    if (block.canDropFromExplosion(this.doppelganger)) {
                        iblockstate.dropBlockAsItemWithChance(this.world, blockpos, 1.0F / this.size, 0);
                    }
                    
                    iblockstate.onBlockExploded(this.world, blockpos, this.doppelganger);
