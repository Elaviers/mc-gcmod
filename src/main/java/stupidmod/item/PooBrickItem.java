    ...
    public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand) {
        if (!world.isRemote) {
            ...
            tst.setLocationAndAngles(
                    player.getPosX() - MathHelper.cos(tst.rotationYaw / 180.0F * (float)Math.PI) * 0.16F,
                    player.getPosY() + player.getEyeHeight() - 0.1,
                    player.getPosZ() - MathHelper.sin(tst.rotationYaw / 180.0F * (float)Math.PI) * 0.16F,
